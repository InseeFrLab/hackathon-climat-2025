...
ds = xr.open_dataset(
    "https://object.files.data.gouv.fr/meteofrance-drias/SocleM-Climat-2025/CPRCM/METROPOLE/ALPX-3/CNRM-ESM2-1/r1i1p1f2/CNRM-AROME46t1/ssp370/day/sfcWindAdjust/version-hackathon-102025/sfcWindAdjust_FR-Metro_CNRM-ESM2-1_ssp370_r1i1p1f2_CNRM-MF_CNRM-AROME46t1_v1-r1_MF-CDFt-SAFRAN-1991-2020_day_20150101-21001231.nc",
    engine="h5netcdf"
)
...
result = (
    ...
    .group_by("year", "y", "x")
    .agg([
        pl.col("target").max().alias("max_wind"),
        (pl.col("target") > 13.8).sum().alias("days_above_50")
    ])
    .sort("year", "y", "x")
)

# result.filter(pl.col("days_above_50")>0)

from cwhpp import create_price_model_pipeline
import lightgbm

col_to_predict = "max_wind"

cols = set(["days_above_50", "max_wind"])
...
joblib.dump(model, "models/temp_max_wind.joblib")
